...
def matchLyrics():
    ...
    with open(data_dir + lyric_dir + "lyrics.jl") as f:
        for item in jl.Reader(f):
            ...
            if song_info:
                new_element = (
                    song_info["title"].lower(),
                    song_info["primary_artist"].replace("-", " ").lower(),
                    item["lyrics"],
                )
    ...
    for index, row in spotify_data.iterrows():
        spotify_dict[
            (row["track_name"].lower(), row["artist_name"].lower())
        ] = index
...
def preprocess(filename):
    ...
    data["time_signature"] = pd.Categorical(
        data.time_signature, ordered=True
    ).codes

    # drop unneeded columns
    data = normalize_cols(
        data, ["popularity", "duration_ms", "loudness", "tempo"]
    )
